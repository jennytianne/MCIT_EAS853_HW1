...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		ERC20 sell = ERC20(sellToken);
        	ERC20 buy = ERC20(sellToken == tokenA ? tokenB : tokenA);

       	 	uint256 sellReserve = sell.balanceOf(address(this));
        	uint256 buyReserve = buy.balanceOf(address(this));
        	uint256 fee = (sellAmount * feebps) / 10000;
        	uint256 buyAmount = buyReserve - (invariant / (sellReserve + (sellAmount - fee)));

        	require(buyAmount > 0 && sell.transferFrom(msg.sender, address(this), sellAmount) && buy.transfer(msg.sender, buyAmount), "Trade failed");
        	invariant = ERC20(tokenA).balanceOf(address(this)) * ERC20(tokenB).balanceOf(address(this));

        	emit Swap(sellToken, address(buy), sellAmount, buyAmount);
	}
	...
	*/
	function provideLiquidity( uint256 amtA, uint256 amtB ) public {
		require( amtA > 0 || amtB > 0, 'Zero liquidity' );
